...
async function loadAnneesScolaires() {
    try {
        const response = await fetch('/api/get-annees-scolaires/');
        const data = await response.json();

        if (data.success && data.annees) {
            const select = document.getElementById('select-annee');
            select.innerHTML = data.annees.map(annee =>
                `<option value="${annee.id}" ${annee.is_current ? 'selected' : ''}>${annee.annee}</option>`
            ).join('');

            // Charger l'historique avec l'année sélectionnée
            if (elements.eleve.value) {
                updateHistorique();
            }
        }
        ...
        elements.grillePaiement.innerHTML = `
            <tr>
                <td colspan="8" class="text-center py-4 text-muted">
                    ...
                    <div class="mt-2">Veuillez sélectionner un élève</div>
                </td>
            </tr>`;
        return;
    }
            ...
            elements.grillePaiement.innerHTML = `
                <tr>
                    <td colspan="8" class="text-center py-4 text-muted">
                        ...
                        <div class="mt-2">Aucun paiement enregistré pour cet élève</div>
                        ${data.has_inscription ? '' : '<div class="alert alert-warning mt-2">Cet élève n\'est pas inscrit pour l\'année sélectionnée</div>'}
                    </td>
                </tr>`;
            return;
        }

        // Afficher le résumé des paiements
        updatePaymentSummary(data);

        // Afficher les paiements avec plus de détails
        // Dans votre fonction qui génère les lignes du tableau
        elements.grillePaiement.innerHTML = data.historique.map(paiement => `
            <tr>
                <td>${paiement.id}</td>
                <td>
                    ${paiement.frais__type_frais === 'Mensuel' ? 
                        `<span class="badge bg-primary">${paiement.mois || 'N/A'}</span>` : 
                        paiement.frais__type_frais === 'Trimestriel' ? 
                        `<span class="badge bg-info">T${paiement.tranche}</span>` : 
                        `<span class="badge bg-success">Annuel</span>`}
                </td>
                <td>
                    <div>${paiement.frais__nom}</div>
                    <small class="text-muted">${paiement.frais__type_frais}</small>
                </td>
                <td class="text-end">${paiement.montant_paye.toFixed(2)} FC</td>
                <td class="text-end">${paiement.solde_restant.toFixed(2)} FC</td>
...
function updatePaymentSummary(data) {
    ...
    summaryElement.innerHTML = `
            ...
            <div class="col-md-4">
                <div class="card border-primary mb-3">
                    <div class="card-body">
                        ...
                        <p class="card-text fs-3">${data.total_paye.toFixed(2)} FC</p>
                    </div>
                </div>
            </div>
            <div class="col-md-8">
                <div class="card border-secondary mb-3">
                    <div class="card-body">
                        <h5 class="card-title">Détails par Type</h5>
                        <div class="row">
                            <div class="col-md-4">
                                <span class="badge bg-primary">Mensuel</span>
                                <p>${data.stats_paiements.mensuel.count} paiements (${data.stats_paiements.mensuel.total.toFixed(2)} FC)</p>
                            </div>
                            <div class="col-md-4">
                                <span class="badge bg-info">Trimestriel</span>
                                <p>${data.stats_paiements.trimestriel.count} paiements (${data.stats_paiements.trimestriel.total.toFixed(2)} FC)</p>
                            </div>
                            <div class="col-md-4">
                                <span class="badge bg-success">Annuel</span>
                                <p>${data.stats_paiements.annuel.count} paiements (${data.stats_paiements.annuel.total.toFixed(2)} FC)</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="alert alert-info">
            <i class="bi bi-info-circle-fill"></i> Historique des paiements pour 
            <strong>${data.eleve_nom}</strong> (${data.eleve_classe}) - 
